...
def _get_mock_spectra() -> tuple[Spectrum, ROOT.TH1D]:
    ...
    # Create mock antineutrino spectrum data
    # Note we have 6 flux values and 7 energy values to represent the edges of 6 bins.
    # We also deliberately include non-uniform bin widths.
    energy = np.array([0, 0.5, 1, 2, 3, 5, 10], dtype=float)
    flux = np.array([10, 20, 30, 40, 50, 60], dtype=float)
    errors = np.array([1, 2, 3, 4, 5, 6], dtype=float)

    # Create the Spectrum instance
    spec = Spectrum(energy, flux, errors, name="mock")

    # Create the ROOT histogram
    root_spec = ROOT.TH1D("mock", "mock", len(energy) - 1, np.array(energy))
    for e, f in zip(energy[:-1], flux, strict=True):
        root_spec.Fill(e, f)
...
def test_create() -> None:
    """Test that Spectrum constructor matches create_spec."""
...
def test_equalise() -> None:
    """Test that Spectrum.equalise matches equalise_spec."""
    spec, root_spec = _get_mock_spectra()
...
def test_scale() -> None:
    """Test that Spectrum scaling matches scale_spec."""
    spec, root_spec = _get_mock_spectra()
...
def test_spec_real() -> None:
    """Test that Spectra from real data matches the ROOT pipeline."""
    max_energy = 6000
    ...
    for isotope, isotope_data in data.items():
        ...
        root_spec = create_spec(energy, flux, errors, name=isotope)
