...
def AddDecays(t, BR, parent_prop0, parent_half_life, daughter_half_life, total_mass = 1000):

    #is branching ratio in correct place

    m_parent_initial = total_mass * parent_prop0 * BR

    lambdaA = np.log(2) / parent_half_life
    lambdaB = np.log(2) / daughter_half_life

    m_daughter_new = (lambdaB / (lambdaB - lambdaA)) * m_parent_initial *(np.exp(-lambdaA*t) - np.exp(-lambdaB*t))
...
def TotSpec(removal_time =0,total_m =1000, max_E=5301, Sr90_prop=0, Y90_prop=0, Pu241_prop=0,Cs137_prop=0, Am242_prop=0, Cm249_prop=0, Cs135_prop=0, I129_prop=0, Np239_prop=0,Tc99_prop=0, Zr93_prop=0, Ce144_prop=0, Kr88_prop=0, Pr144_prop=0, Rb88_prop=0, Rh106_prop=0, Ru106_prop=0):
    ...
    if removal_time != 0:
        extra_Y90 = AddDecays(t = removal_time,BR =1, parent_prop0=Sr90_prop, parent_half_life=half_life_yrs[0], daughter_half_life=half_life_yrs[1], total_mass=total_m)
        spectra.Add(load_equal_scaled(Y90, 2278,"additional Y90" + str(removal_time), isotopes[1], extra_Y90, mr[1], half_life_yrs[1], 0 ))

        extra_Pr144 = AddDecays(t = removal_time, BR=1, parent_prop0=Ce144_prop, parent_half_life=half_life_yrs[11], daughter_half_life=half_life_yrs[13], total_mass=total_m)
        ...
        extra_Rb88 = AddDecays(t = removal_time, BR=1, parent_prop0=Kr88_prop, parent_half_life=half_life_yrs[12], daughter_half_life=half_life_yrs[14], total_mass=total_m)
